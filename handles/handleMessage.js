...
const allowedUsers = [
    '61562826406367', // Identifiant Facebook autorisé
    // Vous pouvez ajouter d'autres identifiants ici
];
...
const handleMessage = async (event, api) => {
    ...
    if (!allowedUsers.includes(senderId.toString())) {
        await sendMessage(senderId, "Désolé, vous n'êtes pas autorisé à utiliser ce bot.");
        return; // Arrête le traitement si l'utilisateur n'est pas autorisé
    }
